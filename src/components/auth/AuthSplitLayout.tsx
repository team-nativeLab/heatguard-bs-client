...
export default function AuthSplitLayout({
  ...
  return (
    <div className="bg-[var(--color-bg-app)] min-h-screen w-full flex relative">
      <div className="hidden md:flex flex-col bg-[var(--color-bg-surface)] border-r border-[var(--color-border)] w-[420px] h-screen shrink-0 relative overflow-hidden px-10 py-10 justify-between">
        <div className="flex gap-2.5 items-center">
          <div className="bg-[#0f172a] flex items-center justify-center rounded size-7 overflow-hidden shrink-0">
        ...
        <div className="pb-0">
          ...
          <div className="font-light text-[var(--color-text-heading)] pt-3">
            {titleLines.map((line) => <p key={line} className={titleClassName}>{line}</p>)}
          </div>
            ...
            <div className="flex flex-col gap-3 pt-8">
              {bullets.map((bullet) => (
                <div key={bullet} className="flex gap-2.5 items-center">
                  ...
                  <p className="text-[var(--color-text-label)] text-sm leading-5">{bullet}</p>
                </div>
              ))}
            </div>
          )}
